...
const SIDE: readonly number[] = TERRAIN.CROSS_SECTION_M;
  ...
  function rebuild(stations: Stations, originX: number, originY: number, originZ: number): void {
    ...
    for (let row = 0; row < rows - 1; row++) {
      for (let col = 0; col < ACROSS - 1; col++) {
        ...
        const c = a + ACROSS;
        const d = c + 1;
        // Alternating the diagonal prevents the entire ribbon resolving into
        // identical slanted strips when triangle normals expose the facets.
        if ((row + col) % 2 === 0) {
          indices[w++] = a;
          indices[w++] = b;
          indices[w++] = d;
